...
export async function createActionItems(
  meetingId: number,
  items: Array<{ title: string; owner?: string; deadline?: Date }>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  return await db.insert(actionItems).values(values);
}
...
export async function saveQARecord(
  ...
  return await db.insert(qaHistory).values({
    meetingId,
    question,
    answer,
  });
...
export async function getQAHistoryByMeeting(meetingId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  return await db
    .select()
    .from(qaHistory)
    .where(eq(qaHistory.meetingId, meetingId));
}
