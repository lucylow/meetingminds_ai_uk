...
function createLLMProvider() {
  const rawBase = ENV.zaiBaseUrl || ENV.forgeApiUrl;

  if (!rawBase) {
    throw new Error("No LLM base URL configured (set ZAI_BASE_URL or BUILT_IN_FORGE_API_URL).");
  }

  const baseURL = rawBase.includes("/api/paas/")
    ? rawBase
    : rawBase.endsWith("/v1")
      ? rawBase
      : `${rawBase}/v1`;
  ...
  return createOpenAI({
    baseURL,
    apiKey: ENV.zaiApiKey || ENV.forgeApiKey,
